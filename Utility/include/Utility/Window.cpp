...
int Window::WindowCallback(UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch (uMsg)
	{
		...
		{
			if (on_window_event) {
				on_window_event(WindowEventType::WINDOW_CLOSE, &WindowEventClose());
			}
		...
		case WM_SIZE:
		{
			...
			if (on_window_event) {
				...
				on_window_event(WindowEventType::WINDOW_RESIZE, &e);
			}
		...
		case WM_KEYDOWN:
		{
			if (on_window_event) {
				...
				on_window_event(WindowEventType::WINDOW_KEYDOWN, &e);
			}

			if ((lParam & (1 << 30)) == 0) {
				if (on_window_event) {
					WindowEventKeyPressed e = { (KeyCode) wParam };
					on_window_event(WindowEventType::WINDOW_KEYPRESSED, &e);
				}
			}
		...
		case WM_KEYUP:
		{
			if (on_window_event) {
				...
				on_window_event(WindowEventType::WINDOW_KEYUP, &e);
			}
			return 1;
		}
	}
